...
Spec JSON:
  {
    ...
    "extra_skill_ids": ["react"],
    "staff_path_fences": {
      "ba-user": ["docs/", "README.md"],
      "git": ["."]
    }
  }
...
Writes path fences into each staff .md + SCOPE.md allow_rw notes.
"""
...
def apply_path_fences(company: Path, fences: dict[str, list]) -> None:
    """Append RW path fence to staff markdown + write SCOPE.md section."""
    staff_files = {p.stem: p for p in list_staff_files(company)}
    scope_lines = [
        "# SCOPE (generated by apply_company_roster)",
        "",
        "Path fences from Add Company wizard. Staffs may only RW listed paths",
    ...
    for name, paths in sorted(fences.items()):
        clean = []
        for raw in paths:
            p = str(raw).strip()
            if p:
        ...
        if not clean:
            continue
        scope_lines.append(f"### `{name}`")
        scope_lines.append("")
        for p in clean:
            scope_lines.append(f"- `{p}`")
        scope_lines.append("")

        path = staff_files.get(name)
        if path is None:
            print(f"warn\tfence_staff_missing\t{name}", file=sys.stderr)
            continue
        text = path.read_text(encoding="utf-8")
        marker = "## Path fence (RW allow)"
        block = (
            f"{marker}\n\n"
            "Staff may only edit these paths (project-relative unless absolute):\n\n"
        ...
        print(f"fence\tstaff\t{name}\t{len(clean)}")

    scope_path = company / "SCOPE.md"
    # Preserve existing SCOPE if present: append generated section
    gen_marker = "# SCOPE (generated by apply_company_roster)"
    generated = "\n".join(scope_lines).rstrip() + "\n"
    if scope_path.is_file():
        old = scope_path.read_text(encoding="utf-8")
        if gen_marker in old:
            start = old.index(gen_marker)
            old = old[:start].rstrip() + "\n\n"
        else:
            old = old.rstrip() + "\n\n"
        scope_path.write_text(old + generated, encoding="utf-8")
    else:
        scope_path.write_text(generated, encoding="utf-8")
    print(f"fence\tscope\t{scope_path}")
