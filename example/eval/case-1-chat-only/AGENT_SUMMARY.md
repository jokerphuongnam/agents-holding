...
Shipped a local Todo MVP: Express JSON API + Vite React frontend. Product code lives in `/Users/phuongnampham/Documents/Agents/eval-todo-bench/case-1-chat-only`.
...
```bash
cd /Users/phuongnampham/Documents/Agents/eval-todo-bench/case-1-chat-only
npm install
npm run install:all
npm run dev
```
